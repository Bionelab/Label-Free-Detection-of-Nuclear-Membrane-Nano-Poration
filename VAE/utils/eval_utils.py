...
def grid_from_2cols(x1, x2, nrow=10, ncol=8, grid_kwargs={"padding": 2, "pad_value": 0.5}):
    """
    Given 2 tensors of images x1, x2, put them in a single image grid (by
    ...
    # Ensure align_corners is not passed to make_grid
    grid_kwargs = {k: v for k, v in grid_kwargs.items() if k != 'align_corners'}
...
def rotate_batch(x, angles):
    """
    Rotate many images by different angles in a batch.
    Args:
...
def rotated_flipped_xs(x, rot_steps, trans=None, do_flip=True, upsample=0):
    ...
    trans: the list of transformation ops to apply. The caller may want to precompute
    ...
    # new tensor to hold a permuted (rotated and possibly flipped) versions, size (n_permutations, bs, c, y, x)
